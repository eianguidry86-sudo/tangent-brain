...
async function geminiJSON(systemPrompt, userPrompt, maxTokens = 1200) {
  const raw = await gemini(
    systemPrompt + "\n\nCRITICAL: Respond with ONLY valid raw JSON. No markdown fences, no backticks, no explanation.",
    userPrompt, maxTokens
  );
  return JSON.parse(raw.replace(/```json|```/g, "").trim());
}

// Drive Mode uses conversation history — separate multi-turn helper
async function geminiChat(history, systemPrompt, maxTokens = 300) {
  const res = await fetch("/api/gemini", {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      model: "gemini-2.0-flash",
  ...
  const data = await res.json();
  if (data.error) throw new Error(data.error.message);
  return data.candidates?.[0]?.content?.parts?.[0]?.text || "Got that.";
}
...
function Pip({ status, color="#60a5fa" }) {
  return <div style={{ width:7, height:7, borderRadius:"50%", flexShrink:0, transition:"all 0.4s",
    background: status==="done"?color:status==="running"?"#fbbf24":"rgba(255,255,255,0.08)",
...
function StatCard({ label, value, unit, color }) {
  return <div style={{ background:`${color}0d`, border:`1px solid ${color}22`, borderRadius:12, padding:"16px 18px" }}>
...
function QCBadge({ severity }) {
  ...
  return <span style={{ display:"inline-flex", alignItems:"center", gap:4, padding:"2px 8px", borderRadius:12,
...
function AgentPill({ label, running, color }) {
  return <div style={{ display:"flex", alignItems:"center", gap:5, padding:"3px 9px", borderRadius:20,
...
// ─── Drive Mode Panel ────────────────────────────────────────────────────────
function DrivePanel({ onSaveEntry }) {
  ...
  const addMsg = (role, text) => setMessages(p => [...p, { role, text, id: genId() }]);
  ...
  const callGemini = async (userMsg) => {
    ...
    const history = msgRef.current.slice(-8).map(m => ({
      role: m.role === "user" ? "user" : "model",
      parts: [{ text: m.text }]
    }));
    // Make sure last message is the new user one
    ...
    try {
      const reply = await geminiChat(
        history,
        `You are Tangent Brain's Drive Mode assistant — voice-first AI for someone driving. Keep ALL responses under 40 words. Be concise, warm, direct. If the user shares a thought or to-do, confirm you captured it and ask if they want to save it. If they say "save this" or "add this", say exactly "Got it, saving now." If they ask a question, answer briefly. Never use markdown, bullets, or lists.`,
        200
      );
      addMsg("model", reply);
      const saveTriggered = reply.toLowerCase().includes("saving now") || reply.toLowerCase().includes("got it, saving");
      if (saveTriggered) setSaveModal({ text: userMsg });
      speak(reply);
    } catch (err) {
      addMsg("model", "Sorry, couldn't connect right now.");
      speak("Sorry, couldn't connect.");
      setState("idle");
    }
  };
  ...
  const handleSave = (type) => {
    if (!saveModal) return;
    onSaveEntry({ id:genId(), raw:saveModal.text, text:saveModal.text, type, timestamp:Date.now(), time:nowTime(), done:false, fromDrive:true,
      clusterId:null, clusterName:null, classifierReasoning:null, classification:null,
      research:null, enrichment:null, synthesis:null, qcScore:null, qcSeverity:null,
      agentStatus:{ classify:"idle", research:"idle", enrich:"idle", output:"idle", qc:"idle" }
    });
    setSaveModal(null);
  };

  const stateLabel = { idle:"Tap orb to speak", listening:"Listening…", processing:"Thinking…", speaking:"Speaking…" };

  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", padding:"28px 20px", height:"100%", overflowY:"auto" }}>
      <div style={{ fontSize:10, color:TEAL, fontFamily:"'Syne Mono',monospace", letterSpacing:"0.12em", marginBottom:8 }}>⚡ DRIVE MODE</div>
      <div style={{ fontSize:13, color:"#4a5a75", marginBottom:28, textAlign:"center", maxWidth:400 }}>
        Hands-free voice chat — speak, I'll respond aloud and capture your thoughts into the pipeline.
      </div>

      {!supported && (
        <div style={{ background:"#2d1a00", border:"1px solid #78350f", borderRadius:10, padding:"12px 16px", marginBottom:20, fontSize:13, color:AMBER, maxWidth:440 }}>
          ⚠️ Speech recognition not supported. Use Chrome or Edge on desktop/Android, or Safari on iOS.
        </div>
      )}

      {/* Orb */}
      <div onClick={state==="idle" ? startListening : stopAll}
        style={{ position:"relative", width:180, height:180, marginBottom:28, cursor:"pointer" }}>
        <div style={{
          width:"100%", height:"100%", borderRadius:"50%",
          background:"radial-gradient(circle at 35% 35%, #2d2a6e, #060810)",
          border:`2px solid ${state==="listening"?TEAL:state==="speaking"?"#a78bfa":"rgba(108,99,255,0.4)"}`,
          display:"flex", alignItems:"center", justifyContent:"center",
          boxShadow: state==="listening"?`0 0 40px rgba(0,212,170,0.3)`:state==="speaking"?"0 0 40px rgba(167,139,250,0.3)":"none",
          animation: state==="listening"?"orbPulse 1.5s ease-in-out infinite":state==="speaking"?"orbPulse2 1s ease-in-out infinite":"none",
          transition:"all 0.3s", position:"relative", overflow:"hidden"
        }}>
          {state==="listening" && [0,1,2].map(i=>(
            <div key={i} style={{ position:"absolute", width:"100%", height:"100%", borderRadius:"50%",
              border:`1px solid rgba(0,212,170,0.3)`, animation:`ringExpand 2s ease-out ${i*0.5}s infinite`, opacity:0 }} />
          ))}
          <span style={{ fontSize:52, zIndex:1 }}>
            {state==="idle"?"🎙️":state==="listening"?"👂":state==="processing"?"🧠":"🔊"}
          </span>
        </div>
      </div>

      <div style={{ fontSize:15, color:"#d8e0f0", marginBottom:6, fontWeight:500 }}>{stateLabel[state]}</div>
      <div style={{ fontSize:12, color:"#3d4a63", marginBottom:24 }}>
        {state==="idle" ? "Tap orb to start · Tap again to stop" : "Tap orb to stop"}
      </div>

      {/* Transcript */}
      {messages.length > 0 && (
        <div ref={transcriptRef} style={{ width:"100%", maxWidth:500, background:"rgba(255,255,255,0.025)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:14, padding:16, minHeight:120, maxHeight:240, overflowY:"auto", marginBottom:12 }}>
          {messages.map(m => (
            <div key={m.id} style={{ marginBottom:10 }}>
              <div style={{ fontSize:10, color:"#2a3a50", fontFamily:"'Syne Mono',monospace", marginBottom:3 }}>
                {m.role==="user" ? "You" : "Tangent"}
              </div>
              <div style={{ padding:"8px 12px", borderRadius:10, fontSize:13, lineHeight:1.5, maxWidth:"85%",
                background: m.role==="user"?"rgba(124,58,237,0.25)":"rgba(58,58,82,0.5)",
                marginLeft: m.role==="user"?"auto":"0", color:"#d8e0f0" }}>
                {m.text}
              </div>
            </div>
          ))}
      ...
      {messages.length > 0 && (
        <div style={{ fontSize:11, color:TEAL, marginBottom:16, fontFamily:"'Syne Mono',monospace" }}>
          Say "save this" to capture a thought into the pipeline
        </div>
      )}

      <div style={{ display:"flex", gap:10 }}>
        {state !== "idle" && (
          <button onClick={stopAll} style={{ padding:"10px 22px", borderRadius:50, border:"none", background:"#ef4444", color:"#fff", fontSize:13, fontWeight:600, cursor:"pointer" }}>
            Stop
          </button>
        )}
        {messages.length > 0 && (
          <button onClick={() => setMessages([])} style={{ padding:"10px 22px", borderRadius:50, border:"1px solid rgba(255,255,255,0.1)", background:"transparent", color:"#4a5a75", fontSize:13, cursor:"pointer" }}>
            Clear
          </button>
        )}
      </div>
      ...
      {saveModal && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200 }}>
          <div style={{ background:"#1a1a26", border:"1px solid rgba(255,255,255,0.1)", borderRadius:16, padding:24, width:360, maxWidth:"90vw" }}>
            <div style={{ fontSize:12, color:"#a78bfa", fontFamily:"'Syne Mono',monospace", marginBottom:14 }}>SAVE TO TANGENT BRAIN</div>
            <div style={{ fontSize:14, color:"#d8e0f0", marginBottom:18, lineHeight:1.6 }}>
              "{saveModal.text}"<br /><br />
              <span style={{ fontSize:12, color:"#4a5a75" }}>This will go through the full 7-agent pipeline.</span>
            </div>
            <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}>
              <button onClick={() => handleSave("thought")} style={{ padding:"8px 16px", borderRadius:8, border:"1px solid rgba(255,255,255,0.1)", background:"transparent", color:"#d8e0f0", fontSize:13, cursor:"pointer" }}>Thought</button>
              <button onClick={() => handleSave("todo")} style={{ padding:"8px 16px", borderRadius:8, border:`1px solid ${AMBER}`, background:"transparent", color:AMBER, fontSize:13, cursor:"pointer" }}>To-Do</button>
              <button onClick={() => handleSave("both")} style={{ padding:"8px 16px", borderRadius:8, border:"none", background:"linear-gradient(135deg,#1d4ed8,#7c3aed)", color:"#f0f4ff", fontSize:13, fontWeight:600, cursor:"pointer" }}>Both</button>
            </div>
          </div>
        </div>
      )}
...
// ─── To-Do Panel ─────────────────────────────────────────────────────────────
function TodoPanel({ thoughts, setThoughts }) {
  const todos    = thoughts.filter(t => t.type==="todo" || t.type==="both" || (t.fromDrive && t.type==="todo"));
  const pending  = todos.filter(t => !t.done);
  const done     = todos.filter(t => t.done);

  const toggle = (id) => setThoughts(p => p.map(t => t.id===id ? {...t, done:!t.done} : t));

  const TodoItem = ({ item }) => {
    const cl = item.clusterName;
    return (
      <div style={{ display:"flex", alignItems:"flex-start", gap:12, padding:14,
        background:"rgba(255,255,255,0.025)", border:"1px solid rgba(255,255,255,0.06)",
        borderRadius:10, marginBottom:8, opacity:item.done?0.5:1, transition:"all 0.2s" }}>
        <div onClick={() => toggle(item.id)} style={{ width:20, height:20, borderRadius:6, flexShrink:0,
          border:`2px solid ${item.done?TEAL:"rgba(255,255,255,0.15)"}`,
          background:item.done?TEAL:"transparent", cursor:"pointer", transition:"all 0.2s",
          display:"flex", alignItems:"center", justifyContent:"center", marginTop:1 }}>
          {item.done && <span style={{ color:"#000", fontSize:12 }}>✓</span>}
        </div>
        <div style={{ flex:1 }}>
          <div style={{ fontSize:14, lineHeight:1.5, color:"#d8e0f0", textDecoration:item.done?"line-through":"none" }}>
            {item.raw || item.text}
          </div>
          <div style={{ fontSize:10, color:"#2a3a50", fontFamily:"'Syne Mono',monospace", marginTop:5, display:"flex", gap:8, flexWrap:"wrap" }}>
            <span>{item.type==="both"?"thought + to-do":"to-do"}</span>
            {item.fromDrive && <span style={{ color:TEAL }}>via Drive Mode</span>}
            {cl && <span style={{ color:"#a78bfa" }}>{cl}</span>}
  ...
  return (
    <div style={{ padding:"24px 28px", overflowY:"auto", height:"calc(100vh - 120px)" }}>
      {/* Stats */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:24 }}>
        <StatCard label="PENDING"   value={pending.length} unit="" color={AMBER} />
        <StatCard label="COMPLETED" value={done.length}    unit="" color={TEAL}  />
        <StatCard label="TOTAL"     value={todos.length}   unit="" color="#a78bfa" />
      </div>

      {todos.length === 0 && (
        <div style={{ textAlign:"center", color:"#1e2a3a", paddingTop:60 }}>
          <div style={{ fontSize:36, marginBottom:12 }}>☐</div>
          <div style={{ fontFamily:"'Syne Mono',monospace", fontSize:11, marginBottom:6 }}>No to-dos yet</div>
          <div style={{ fontSize:11, color:"#1a2230" }}>
            Tag a capture as "to-do" or "both", or say "save this" in Drive Mode
          </div>
        </div>
      )}
      ...
      {pending.length > 0 && (
        <div style={{ marginBottom:24 }}>
          <SectionLabel color="#2a3a50">PENDING ({pending.length})</SectionLabel>
      ...
      {done.length > 0 && (
        <div>
          <SectionLabel color="#2a3a50">COMPLETED ({done.length})</SectionLabel>
          {done.map(item => <TodoItem key={item.id} item={item} />)}
        </div>
      )}
    </div>
  );
...
export default function TangentBrain() {
  ...
  const [entryType, setEntryType]         = useState("thought"); // thought|todo|both
  ...
  async function runClassifier(id, raw) {
    ...
    try {
      ...
      const p = await GJ(
        "You are a semantic clustering agent for a personal thought-capture system.",
        `Existing clusters:\n${existing}\n\nNew thought: "${raw}"\n\nMatch if genuinely same domain. Names must be personal+specific. Only create new cluster if truly different domain.\n\nRespond ONLY:\n{"action":"match or create","clusterId":"existing id if match else empty","newClusterName":"name if creating","newClusterExampleTopics":"topic1,topic2","reasoning":"one sentence why"}`
      );
      let cid, cname;
  ...
  async function runResearcher(id, raw) {
    ...
    try {
      const res = await G("Personal knowledge research agent. Rich but scannable. Max 350 words.", `Thought: "${raw}"`);
      updT(id, { research:res });
    } catch(e) { updT(id, { research:"Error: "+e.message }); }
  ...
  async function runEnricher(id, raw, cname) {
    ...
    try {
      ...
      const res = await G(
        "Thought enrichment agent. Cross-cluster connections, 2-3 unexplored angles, one rabbit-hole question. Max 300 words.",
        `New thought: "${raw}"\nCluster: ${cname||"?"}\n\nPast thoughts:\n${ctx||"(first thought)"}`
      );
      updT(id, { enrichment:res });
    } catch(e) { updT(id, { enrichment:"Error: "+e.message }); }
  ...
  async function runSynthesizer(id, raw) {
    ...
    try {
      ...
      const res = await G(
        "Synthesis agent. Format:\n## Core Insight\n## Why This Matters To You\n## Unexpected Angle\n## Action Items\n## Come Back To This When...\nMax 280 words.",
        `Thought: "${raw}"\nResearch: ${(t?.research||"").slice(0,350)}\nEnrichment: ${(t?.enrichment||"").slice(0,350)}`
      );
      updT(id, { synthesis:res });
    } catch(e) { updT(id, { synthesis:"Error: "+e.message }); }
  ...
  async function runQCOnThought(id) {
    ...
    try {
      ...
      const parsed = await GJ(
        "Strict QC agent auditing a 4-agent pipeline.",
        `THOUGHT: "${t?.raw}"\nCLUSTER: "${cluster?.name||"Unassigned"}" | Reasoning: "${t?.classifierReasoning||"none"}"\nAll clusters: ${allClusters||"none"}\nRESEARCHER: "${(t?.research||"").slice(0,400)}"\nENRICHER: "${(t?.enrichment||"").slice(0,400)}"\nSYNTHESIZER: "${(t?.synthesis||"").slice(0,400)}"\n\nReturn JSON:\n{"thoughtId":"${id}","overallScore":0.0,"overallSeverity":"pass","summary":"2 sentences","clusterCorrect":true,"clusterNote":"","suggestedCluster":"","agents":{"classifier":{"accuracy":0.9,"relevance":0.9,"quality":0.9,"severity":"pass","issues":[],"suggestion":""},"researcher":{"accuracy":0.9,"relevance":0.9,"quality":0.9,"severity":"pass","issues":[],"suggestion":""},"enricher":{"accuracy":0.9,"relevance":0.9,"quality":0.9,"severity":"pass","issues":[],"suggestion":""},"synthesizer":{"accuracy":0.9,"relevance":0.9,"quality":0.9,"severity":"pass","issues":[],"suggestion":""}},"checkedAt":${Date.now()}}`
      );
  ...
  async function runScout() {
    ...
    setScoutRunning(true); notify("Scout searching via Google…");
    try {
      const raw = await GS(
        `Youth basketball intelligence scout. Search and return ONLY a JSON array. Each item: {"title":"","summary":"2-3 sentences","category":"Stats|Business|Coaching|Laws & Policy|Rankings|Recruitment|Community|Technology|Health|Other","keyStats":["stat"],"source":"","relevance":"local|national|international","date":"","url":""}. Return ONLY raw JSON array.`,
        "Search for latest youth basketball news, stats, business developments, coaching strategies, laws or policies, rankings, and recruitment stories from the past 2 weeks."
      );
      let articles = [];
      try { const m = raw.match(/\[[\s\S]*?\]/s); articles = m ? JSON.parse(m[0]) : []; } catch {}
      setScoutArticles(articles.map((a,i) => ({...a, id:`art_${Date.now()}_${i}`, fetchedAt:Date.now()})));
      setScoutLastRun(Date.now());
      notify(`Scout found ${articles.length} articles`);
      await runQCOnScout(articles);
  ...
  async function runDigest() {
    ...
    setDigestRunning(true); notify("Digest pulling today's data…");
    try {
      const raw = await GS(
        `Daily youth basketball digest agent. Return ONLY raw JSON: {"digestDate":"today","headline":"biggest story","changes":[{"topic":"","change":"","direction":"up|down|new","magnitude":""}],"extractedStats":[{"stat":"","value":"","unit":"","category":"Stats|Business|Coaching|Laws & Policy|Rankings|Recruitment|Community|Technology|Health","context":"","date":""}],"topStories":[{"title":"","summary":"2 sentences","category":"","isNew":true}]}`,
        "Search for today's youth basketball updates: new stats, rule changes, tournament results, participation numbers, market data, technology trends."
      );
      let digest = null;
      ...
      if (digest) {
        ...
        if (digest.extractedStats?.length) {
          setStatsDb(p => [...digest.extractedStats.map((s,i)=>({...s,id:`st_${Date.now()}_${i}`,addedAt:Date.now()})),...p].slice(0,200));
        }
  ...
  async function runQCOnScout(articles) {
    ...
    try {
      const sample = articles.slice(0,6).map(a=>`"${a.title}" | ${a.category} | ${a.source}`).join("\n");
      const parsed = await GJ("JSON-only QC agent.",
        `Audit scout output (${articles.length} articles):\n${sample}\n\nReturn JSON:\n{"scoutScore":0.0,"scoutSeverity":"pass|warn|flag|fail","categoryIssues":[],"statIssues":[],"diversityNote":"","relevanceNote":"","suggestions":[],"checkedAt":${Date.now()}}`
      );
  ...
  async function runFullQCSweep() {
    if (qcRunning) return;
    setQcRunning(true); notify("QC full system sweep…");
    try {
      const allT = tRef.current; const allCl = clRef.current; const allSt = stRef.current;
      const clInput = allT.slice(0,12).map(t=>`"${t.raw?.slice(0,80)}" → "${clRef.current.find(c=>c.id===t.clusterId)?.name||"unassigned"}"`).join("\n");
      const clAudit = await GJ("JSON-only QC.",
      ...
      const stAudit = allSt.length > 0 ? await GJ("JSON-only QC.",
        `Audit stats:\n${allSt.slice(0,10).map(s=>`"${s.stat}": ${s.value}${s.unit?" "+s.unit:""} (${s.category})`).join("\n")}\n\nReturn JSON:\n{"statsScore":0.0,"severity":"pass|warn|flag|fail","flaggedStats":[{"stat":"","issue":"","severity":"warn"}],"duplicates":[],"note":"","checkedAt":${Date.now()}}`
      ) : { statsScore:1.0, severity:"pass", flaggedStats:[], duplicates:[], note:"No stats", checkedAt:Date.now() };
      const completed = allT.filter(t=>t.synthesis&&t.research);
      const conAudit = completed.length > 0 ? await GJ("JSON-only QC.",
        `Audit cross-agent consistency:\n${completed.slice(0,4).map(t=>`Thought: "${t.raw?.slice(0,60)}"\nResearch: "${(t.research||"").slice(0,100)}"\nSynthesis: "${(t.synthesis||"").slice(0,100)}"`).join("\n---\n")}\n\nReturn JSON:\n{"consistencyScore":0.0,"severity":"pass|warn|flag|fail","inconsistencies":[{"thought":"","issue":"","agents":""}],"overallNote":"","checkedAt":${Date.now()}}`
      ) : { consistencyScore:1.0, severity:"pass", inconsistencies:[], overallNote:"No completed thoughts", checkedAt:Date.now() };
  ...
  const todos          = thoughts.filter(t=>t.type==="todo"||t.type==="both");
  ...
  const TABS = [
    ...
    { id:"drive",    label:"🚗 Drive",                             icon:"",  accent:TEAL },
    { id:"todo",     label:`To-Do ${pendingTodos.length>0?`(${pendingTodos.length})`:""}`, icon:"☐", accent:AMBER },
    ...
    <div style={{ minHeight:"100vh", background:"#060810", fontFamily:"'DM Sans',sans-serif", color:"#d8e0f0", display:"flex", flexDirection:"column", position:"relative" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        @keyframes orbPulse2{0%,100%{transform:scale(1)}50%{transform:scale(1.02)}}
      ...
      <div style={{ position:"relative",zIndex:10,background:"rgba(6,8,16,0.87)",backdropFilter:"blur(14px)",borderBottom:"1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ display:"flex",alignItems:"center",gap:14,padding:"16px 28px 0",flexWrap:"wrap" }}>
          ...
          <div>
            ...
            <div style={{ fontSize:10,color:"#1e2a3a",fontFamily:"'Syne Mono',monospace",letterSpacing:"0.05em" }}>9-TAB · 7 AGENTS · GEMINI 2.0 FLASH · PERSISTENT</div>
          </div>
        ...
        <div style={{ display:"flex",padding:"0 28px",gap:0,overflowX:"auto" }}>
          {TABS.map(t=>(
            <button key={t.id} className="tb" onClick={()=>setTab(t.id)}
              style={{ padding:"9px 14px",background:"none",border:"none",borderBottom:`2px solid ${tab===t.id?(t.accent||"#60a5fa"):"transparent"}`,color:tab===t.id?(t.accent||"#93c5fd"):"#2a3a50",fontSize:12,fontWeight:tab===t.id?500:400,transition:"all 0.2s",whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:5 }}>
      ...
      <div style={{ flex:1,position:"relative",zIndex:1,overflow:"hidden" }}>
        ...
        {tab==="capture" && (
          <div style={{ maxWidth:580,margin:"36px auto",padding:"0 24px",animation:"fu 0.3s ease" }}>
            <Panel style={{ padding:26,marginBottom:16 }}>
              <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14 }}>
                ...
                <div style={{ display:"flex",gap:6 }}>
                  {[["thought","THOUGHT","#a78bfa"],["todo","TO-DO",AMBER],["both","BOTH","#d4af37"]].map(([val,label,color])=>(
                    <button key={val} onClick={()=>setEntryType(val)} style={{ fontSize:10,fontFamily:"'Syne Mono',monospace",padding:"3px 10px",borderRadius:20,border:`1px solid ${entryType===val?color:"rgba(255,255,255,0.1)"}`,background:entryType===val?`${color}18`:"transparent",color:entryType===val?color:"#3d4a63",cursor:"pointer",transition:"all 0.2s" }}>
                      {label}
                    </button>
                  ))}
                </div>
              ...
              <textarea value={input} onChange={e=>setInput(e.target.value)}
                ...
                placeholder={entryType==="todo"?"What needs to get done?":entryType==="both"?"A thought that's also an action…":"You heard something and your brain went somewhere..."}
            ...
            <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:16 }}>
              <StatCard label="THOUGHTS"  value={thoughts.length}    unit="" color="#a78bfa" />
              <StatCard label="PENDING"   value={pendingTodos.length} unit="" color={AMBER}   />
              <StatCard label="CLUSTERS"  value={clusters.length}    unit="" color="#34d399"  />
            </div>

            <Panel style={{ padding:"20px 22px" }}>
              <SectionLabel>9 TABS · 7 AGENTS</SectionLabel>
              {[
                {c:"#a78bfa",i:"⬡",n:"Classifier",   d:"Emergent clustering — matches or creates domains from your actual thinking patterns."},
                {c:"#60a5fa",i:"◎",n:"Researcher",   d:"Deep research brief on any thought you capture."},
                {c:"#f472b6",i:"◈",n:"Enricher",     d:"Cross-references all past thoughts, surfaces unexpected connections."},
                {c:"#34d399",i:"◆",n:"Synthesizer",  d:"Clean Thought Report with insight, angles, and action items."},
                {c:TEAL,     i:"🚗",n:"Drive Mode",  d:"Hands-free voice capture while driving. Say 'save this' to send through the pipeline."},
                {c:AMBER,    i:"☐", n:"To-Do",       d:"All thoughts tagged as to-do or both, with pending/done tracking."},
                {c:"#38bdf8",i:"⬡",n:"Scout",        d:"Google Search grounded youth basketball news, categorized by topic."},
                {c:"#fbbf24",i:"▣",n:"Daily Digest", d:"Today's changes extracted into a structured Stats DB."},
                {c:QC_COLOR, i:"⊛",n:"QC Agent",     d:"Audits every agent for accuracy, cluster correctness, and consistency."},
              ].map(a=>(
                <div key={a.n} style={{ display:"flex",gap:12,marginBottom:12 }}>
                  <div style={{ width:28,height:28,borderRadius:7,flexShrink:0,background:`${a.c}12`,border:`1px solid ${a.c}28`,display:"flex",alignItems:"center",justifyContent:"center",color:a.c,fontSize:12 }}>{a.i}</div>
                  <div><div style={{ fontSize:12,fontWeight:600,color:a.c,marginBottom:2,fontFamily:"'Syne',sans-serif" }}>{a.n}</div><div style={{ fontSize:11,color:"#3d4a63",lineHeight:1.6 }}>{a.d}</div></div>
                </div>
              ))}
            </Panel>
          </div>
                ...
                return (
                  <div key={t.id} className="hr" onClick={()=>setSelectedId(t.id)}
                    ...
                    <div style={{ display:"flex",gap:6,alignItems:"flex-start",marginBottom:7 }}>
                      <div style={{ fontSize:12,color:isSel?"#d8e0f0":"#6b7fa0",lineHeight:1.5,flex:1 }}>{(t.raw||t.text||"").slice(0,65)}{(t.raw||t.text||"").length>65?"…":""}</div>
                    ...
                    <div style={{ fontSize:9,color:"#1e2a3a",marginTop:5,fontFamily:"'Syne Mono',monospace",display:"flex",gap:8 }}>
                      <span>{ago(t.timestamp)}</span>
                      {t.type && t.type!=="thought" && <span style={{ color:t.type==="todo"?AMBER:t.type==="both"?"#d4af37":TEAL }}>{t.type}</span>}
                      {t.fromDrive && <span style={{ color:TEAL }}>drive</span>}
                    </div>
                  </div>
                );
              })}
                ...
                : <div style={{ animation:"fu 0.3s ease",maxWidth:700 }}>
                    <Panel style={{ padding:"18px 22px",marginBottom:20,borderColor:selCluster?`${selCluster.color}22`:"rgba(255,255,255,0.06)" }}>
                      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,marginBottom:10 }}>
                        <p style={{ fontSize:15,color:"#d8e0f0",lineHeight:1.7,fontStyle:"italic",flex:1 }}>"{selected.raw||selected.text}"</p>
                        <div style={{ display:"flex",flexDirection:"column",gap:6,alignItems:"flex-end" }}>
                          ...
                          {selected.type && selected.type!=="thought" && <Badge label={selected.type} color={selected.type==="todo"?AMBER:"#d4af37"} small />}
                        </div>
                      </div>
                      ...
                      <div style={{ fontSize:9,color:"#1e2a3a",marginTop:7,fontFamily:"'Syne Mono',monospace" }}>{new Date(selected.timestamp).toLocaleString()}{selected.fromDrive?" · via Drive Mode":""}</div>
                    </Panel>
                    {[
                    ...
                    <div style={{ marginBottom:18 }}>
                      ...
                      <Panel style={{ padding:"14px 16px",minHeight:56,borderColor:`${QC_COLOR}18` }}>
                        ...
                        {selQcReport && (
                          <div>
                            <div style={{ display:"flex",gap:10,alignItems:"center",marginBottom:12 }}><QCBadge severity={selQcReport.overallSeverity} /><ConfidenceBar score={selQcReport.overallScore} /></div>
              ...
              : <>
                  <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))",gap:12,marginBottom:28 }}>
                    {clustersSorted.map((cl,i)=>(
                      <div key={cl.id} style={{ background:`${cl.color}0a`,border:`1px solid ${cl.color}25`,borderRadius:13,padding:"16px 18px",cursor:"pointer",transition:"all 0.2s" }}
                          ...
                          <div style={{ paddingLeft:16,borderLeft:`1px solid ${cl.color}28` }}>
                            {cts.map(t=>(
                              <div key={t.id} className="hr" onClick={()=>{setSelectedId(t.id);setTab("thoughts");}}
                                ...
                                <span style={{ fontSize:12,color:"#c4cfdf",lineHeight:1.5 }}>{(t.raw||t.text||"").slice(0,85)}{(t.raw||t.text||"").length>85?"…":""}</span>
        ...
        {/* DRIVE MODE */}
        {tab==="drive" && (
          <div style={{ height:"calc(100vh - 120px)",overflow:"hidden",animation:"fu 0.3s ease" }}>
            <DrivePanel onSaveEntry={t => { setThoughts(p => [t,...p]); runPipeline(t); notify("Thought captured from Drive Mode — pipeline running…"); }} />
          </div>
        )}
        ...
        {tab==="todo" && (
          <div style={{ animation:"fu 0.3s ease" }}>
            <TodoPanel thoughts={thoughts} setThoughts={setThoughts} />
          </div>
        )}
          ...
          <div style={{ height:"calc(100vh - 120px)",display:"flex",flexDirection:"column",animation:"fu 0.3s ease" }}>
            <div style={{ padding:"14px 28px",borderBottom:"1px solid rgba(255,255,255,0.05)",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap" }}>
              <button onClick={runScout} disabled={scoutRunning}
                ...
                {scoutRunning?<><Spinner color="#38bdf8" />Searching…</>:"⬡ Run Scout Agent"}
              </button>
            ...
            <div style={{ flex:1,overflowY:"auto",padding:"20px 28px" }}>
              {scoutArticles.length===0&&!scoutRunning&&<div style={{ textAlign:"center",color:"#1e2a3a",paddingTop:80 }}><div style={{ fontSize:38,marginBottom:14 }}>⬡</div><div style={{ fontFamily:"'Syne Mono',monospace",fontSize:11 }}>Scout hasn't run yet</div></div>}
                ...
                {filteredArts.map(art=>{
                  ...
                  return (
                    <div key={art.id} className="art" style={{ background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:12,padding:"16px 18px",transition:"all 0.2s" }}>
                      <div style={{ display:"flex",justifyContent:"space-between",gap:10,marginBottom:10 }}><Badge label={art.category} color={col} small /><span style={{ fontSize:9,color:"#2a3a50",fontFamily:"'Syne Mono',monospace" }}>{art.relevance}</span></div>
                      ...
                      {art.keyStats?.length>0 && <div style={{ borderTop:"1px solid rgba(255,255,255,0.05)",paddingTop:10 }}>{art.keyStats.map((s,i)=><div key={i} style={{ fontSize:10,color:col,fontFamily:"'Syne Mono',monospace",marginBottom:3,display:"flex",gap:6 }}><span style={{opacity:0.5}}>▸</span>{s}</div>)}</div>}
                      <div style={{ display:"flex",justifyContent:"space-between",marginTop:10,fontSize:9,color:"#1e2a3a",fontFamily:"'Syne Mono',monospace" }}><span>{art.source}</span><span>{art.date}</span></div>
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{ height:"calc(100vh - 120px)",display:"flex",flexDirection:"column",animation:"fu 0.3s ease" }}>
            <div style={{ padding:"14px 28px",borderBottom:"1px solid rgba(255,255,255,0.05)",display:"flex",alignItems:"center",gap:12 }}>
              <button onClick={runDigest} disabled={digestRunning}
                style={{ padding:"8px 20px",borderRadius:8,border:"none",background:digestRunning?"rgba(52,211,153,0.1)":"linear-gradient(135deg,#065f46,#047857)",color:digestRunning?"#34d399":"#f0f4ff",fontSize:12,fontWeight:500,display:"flex",alignItems:"center",gap:7,transition:"all 0.25s" }}>
                {digestRunning?<><Spinner color="#34d399" />Compiling…</>:"◆ Run Daily Digest"}
              </button>
              {digestLastRun && <span style={{ fontSize:10,color:"#2a3a50",fontFamily:"'Syne Mono',monospace" }}>Last run {ago(digestLastRun)}</span>}
            </div>
            <div style={{ flex:1,overflowY:"auto",padding:"20px 28px" }}>
              {digestEntries.length===0&&!digestRunning&&<div style={{ textAlign:"center",color:"#1e2a3a",paddingTop:80 }}><div style={{ fontSize:38,marginBottom:14 }}>◆</div><div style={{ fontFamily:"'Syne Mono',monospace",fontSize:11 }}>No digest runs yet</div></div>}
                ...
                <div key={entry.id} style={{ marginBottom:28,animation:"fu 0.3s ease" }}>
                  ...
                  {entry.changes?.length>0&&(
                    <div style={{ marginBottom:14 }}>
                      ...
                      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:8 }}>
                        {entry.changes.map((ch,i)=>{ const isUp=ch.direction==="up";const isNew=ch.direction==="new";const col=isNew?"#a78bfa":isUp?"#34d399":"#f87171";return(<Panel key={i} style={{ padding:"12px 14px",borderColor:`${col}20` }}><div style={{ fontSize:10,color:col,fontFamily:"'Syne Mono',monospace",marginBottom:5 }}>{isNew?"✦ NEW":isUp?"↑ UP":"↓ DOWN"}{ch.magnitude?` · ${ch.magnitude}`:""}</div><div style={{ fontSize:12,fontWeight:500,color:"#c4cfdf",marginBottom:3 }}>{ch.topic}</div><div style={{ fontSize:11,color:"#4a5a75",lineHeight:1.5 }}>{ch.change}</div></Panel>);})}
                      </div>
                    </div>
                  )}
                  {entry.topStories?.length>0&&(
                    <div>
                      ...
                      {entry.topStories.map((s,i)=>{ const col=SCOUT_COLORS[s.category]||"#94a3b8";return(<div key={i} style={{ display:"flex",gap:12,marginBottom:10,padding:"10px 12px",borderRadius:9,background:"rgba(255,255,255,0.02)",borderLeft:`3px solid ${col}` }}><div style={{ flex:1 }}><div style={{ display:"flex",gap:8,alignItems:"center",marginBottom:5 }}><Badge label={s.category} color={col} small />{s.isNew&&<span style={{ fontSize:9,color:"#a78bfa",fontFamily:"'Syne Mono',monospace" }}>NEW</span>}</div><div style={{ fontSize:13,fontWeight:500,color:"#c4cfdf",marginBottom:4,fontFamily:"'Syne',sans-serif" }}>{s.title}</div><div style={{ fontSize:11,color:"#4a5a75",lineHeight:1.5 }}>{s.summary}</div></div></div>);})}
                    </div>
                  )}
        ...
        {tab==="statsdb" && (
          <div style={{ height:"calc(100vh - 120px)",overflowY:"auto",padding:"24px 28px",animation:"fu 0.3s ease" }}>
            {statsDb.length===0?<div style={{ textAlign:"center",color:"#1e2a3a",paddingTop:80 }}><div style={{ fontSize:38,marginBottom:14 }}>▣</div><div style={{ fontFamily:"'Syne Mono',monospace",fontSize:11,marginBottom:8 }}>Stats database empty</div><div style={{ fontSize:11,color:"#1a2230" }}>Run the Daily Digest to populate</div></div>:(
              <>
                ...
                <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:10,marginBottom:28 }}>
                  {statsByCat.map(([cat,stats])=>{ const col=SCOUT_COLORS[cat]||"#94a3b8";const pct=Math.round((stats.length/statsDb.length)*100);return(<Panel key={cat} style={{ padding:"14px 16px" }}><div style={{ display:"flex",justifyContent:"space-between",marginBottom:8 }}><span style={{ fontSize:11,color:col,fontFamily:"'Syne Mono',monospace" }}>{cat}</span><span style={{ fontSize:11,color:"#2a3a50",fontFamily:"'Syne Mono',monospace" }}>{stats.length} · {pct}%</span></div><div style={{ height:3,background:"rgba(255,255,255,0.04)",borderRadius:2,overflow:"hidden",marginBottom:10 }}><div style={{ height:"100%",width:`${pct}%`,background:col,transition:"width 1s ease" }} /></div>{stats[0]&&<div style={{ fontSize:10,color:"#4a5a75",fontFamily:"'Syne Mono',monospace",lineHeight:1.5 }}>Latest: <span style={{color:col}}>{stats[0].value}{stats[0].unit?" "+stats[0].unit:""}</span> — {stats[0].stat}</div>}</Panel>);})}
                </div>
                <SectionLabel color="#2a3a50">ALL STATS ({statsDb.length})</SectionLabel>
                <Panel style={{ overflow:"hidden" }}>
                  <div style={{ overflowX:"auto" }}>
                    <table style={{ width:"100%",borderCollapse:"collapse",fontSize:12 }}>
                      ...
                      <tbody>{statsDb.map((s,i)=>{ const col=SCOUT_COLORS[s.category]||"#94a3b8";return(<tr key={s.id} style={{ borderBottom:"1px solid rgba(255,255,255,0.04)",background:i%2===0?"transparent":"rgba(255,255,255,0.01)" }}><td style={{ padding:"9px 14px",color:"#c4cfdf",fontWeight:500,maxWidth:200 }}>{s.stat}</td><td style={{ padding:"9px 14px",color:col,fontFamily:"'Syne Mono',monospace",fontWeight:700,whiteSpace:"nowrap" }}>{fmt(s.value)}{s.unit?" "+s.unit:""}</td><td style={{ padding:"9px 14px" }}><Badge label={s.category} color={col} small /></td><td style={{ padding:"9px 14px",color:"#4a5a75",maxWidth:260,lineHeight:1.5 }}>{s.context}</td><td style={{ padding:"9px 14px",color:"#2a3a50",fontFamily:"'Syne Mono',monospace",fontSize:10,whiteSpace:"nowrap" }}>{s.date}</td></tr>);})}</tbody>
                    </table>
                  </div>
                </Panel>
              </>
          ...
          <div style={{ height:"calc(100vh - 120px)",display:"flex",flexDirection:"column",animation:"fu 0.3s ease" }}>
            <div style={{ padding:"14px 28px",borderBottom:"1px solid rgba(255,255,255,0.05)",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap" }}>
              <button onClick={runFullQCSweep} disabled={qcRunning}
                style={{ padding:"8px 20px",borderRadius:8,border:"none",background:qcRunning?`${QC_COLOR}15`:"linear-gradient(135deg,#7c2d12,#c2410c)",color:qcRunning?QC_COLOR:"#f0f4ff",fontSize:12,fontWeight:500,display:"flex",alignItems:"center",gap:7,transition:"all 0.25s" }}>
                {qcRunning?<><Spinner color={QC_COLOR} />Auditing…</>:"⊛ Run Full QC Sweep"}
              </button>
            ...
            <div style={{ flex:1,overflowY:"auto",padding:"20px 28px" }}>
              <div style={{ marginBottom:28 }}>
                ...
                <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(155px,1fr))",gap:10,marginBottom:20 }}>
                  <StatCard label="AUDITED"    value={qcStats.total}                           unit=""  color={QC_COLOR}   />
                  <StatCard label="AVG SCORE"  value={`${Math.round((qcStats.avg||0)*100)}`}  unit="%" color="#34d399"    />
                  <StatCard label="FLAGS"      value={qcStats.flagged}                         unit=""  color="#f87171"    />
                  <StatCard label="REPORTS"    value={qcReports.length}                        unit=""  color="#a78bfa"    />
                </div>
                {sweep && (
                  <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14,marginBottom:24 }}>
                    {[
                      ...
                      <Panel key={label} style={{ padding:"18px 20px",borderColor:`${QC_COLOR}18` }}>
                        ...
                        {data?.[issuesKey]?.length>0&&<div style={{ marginTop:10 }}>{data[issuesKey].slice(0,3).map((item,i)=><div key={i} style={{ padding:"6px 8px",borderRadius:6,background:`${QC_COLOR}08`,border:`1px solid ${QC_COLOR}18`,marginBottom:5,fontSize:10,color:"#8a7a6a",lineHeight:1.4 }}>{item.thought||item.stat||""}{(item.shouldBe||item.issue)&&<span style={{color:`${QC_COLOR}99`}}> → {item.shouldBe||item.issue}</span>}</div>)}</div>}
                      </Panel>
                    ))}
                    {scoutQc&&(
              ...
              {qcReports.length>0&&(
                <div>
                  <SectionLabel color="#2a3a50">PER-THOUGHT QC ({qcReports.length})</SectionLabel>
                  <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
                    {qcReports.map(r=>{
                      const t=thoughts.find(th=>th.id===r.thoughtId); const isExp=expandedQcId===r.thoughtId;
                      return (
                        <Panel key={r.thoughtId} style={{ overflow:"hidden",borderColor:isExp?`${QC_COLOR}25`:"rgba(255,255,255,0.06)" }}>
                          <div style={{ padding:"14px 18px",cursor:"pointer",display:"flex",alignItems:"center",gap:12 }} onClick={()=>setExpandedQcId(isExp?null:r.thoughtId)}>
                            ...
                            <div style={{ flex:1,minWidth:0 }}>
                              <div style={{ fontSize:12,color:"#c4cfdf",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{t?.raw||t?.text||r.thoughtId}</div>
                            ...
                            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:4 }}>
                              {Object.entries(r.agents||{}).map(([name,agQc])=>(<div key={name} style={{ display:"flex",alignItems:"center",gap:4 }}><div style={{ width:5,height:5,borderRadius:"50%",background:SEV[agQc.severity]?.color||"#94a3b8" }} /><span style={{ fontSize:9,color:"#2a3a50",fontFamily:"'Syne Mono',monospace" }}>{name.slice(0,5)}</span></div>))}
                            </div>
                          ...
                          {isExp&&(
                            <div style={{ padding:"0 18px 16px",borderTop:"1px solid rgba(255,255,255,0.05)",paddingTop:14,animation:"fu 0.2s ease" }}>
                              ...
                              {!r.clusterCorrect&&<div style={{ padding:"8px 12px",borderRadius:8,background:`${QC_COLOR}0a`,border:`1px solid ${QC_COLOR}1a`,marginBottom:12 }}><div style={{ fontSize:10,color:QC_COLOR,fontFamily:"'Syne Mono',monospace",marginBottom:4 }}>CLUSTER MISMATCH</div><div style={{ fontSize:11,color:"#7a6a5a" }}>{r.clusterNote}</div>{r.suggestedCluster&&<div style={{ fontSize:11,color:`${QC_COLOR}99`,marginTop:3 }}>Suggested: "{r.suggestedCluster}"</div>}</div>}
                              <div style={{ display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8 }}>
                                {Object.entries(r.agents||{}).map(([name,agQc])=>(
                                  <div key={name} style={{ padding:"10px 12px",borderRadius:9,background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.05)" }}>
                                    <div style={{ display:"flex",justifyContent:"space-between",marginBottom:8 }}><span style={{ fontSize:10,color:"#4a5a75",fontFamily:"'Syne Mono',monospace",textTransform:"uppercase" }}>{name}</span><QCBadge severity={agQc.severity} /></div>
                                    {[["Accuracy",agQc.accuracy],["Relevance",agQc.relevance],["Quality",agQc.quality]].map(([l,v])=>(<div key={l} style={{ display:"flex",alignItems:"center",gap:6,marginBottom:4 }}><span style={{ fontSize:9,color:"#2a3a50",fontFamily:"'Syne Mono',monospace",width:52 }}>{l}</span><ConfidenceBar score={v||0} /></div>))}
                              ...
                              <button onClick={()=>{setSelectedId(r.thoughtId);setTab("thoughts");}} style={{ marginTop:12,padding:"6px 14px",borderRadius:7,border:`1px solid ${QC_COLOR}30`,background:`${QC_COLOR}0a`,color:QC_COLOR,fontSize:11,fontFamily:"'Syne Mono',monospace" }}>
                                View Full Thought →
                              </button>
                            </div>
                          )}
              ...
              {qcReports.length===0&&!sweep&&<div style={{ textAlign:"center",color:"#1e2a3a",paddingTop:60 }}><div style={{ fontSize:38,marginBottom:14,color:`${QC_COLOR}44` }}>⊛</div><div style={{ fontFamily:"'Syne Mono',monospace",fontSize:11,marginBottom:8,color:"#2a3a50" }}>QC hasn't run yet</div><div style={{ fontSize:11,color:"#1a2230" }}>Capture a thought to trigger automatic QC</div></div>}
            </div>
          </div>
        )}
      </div>
